...
entradas derivadas.
**Note** se puede usar cualquier otra funcion no linea, tangente por ejemplo
...
**Note** entropy = T, es para regresion logistica?? (verificar)
...
**Note** Back propagation - se utiliza para calcular el gradiente de la red, para posteriormente minimizar la devianza
...
**Note class** Ya determinadas las variables de una capa, los valores de las capas anteriores ya no importan, las variables de las capas posteriores se pueden determinar de las variables determinadas. 
...
# Modelo loss - estamos minimizando la devianza con los pesos penalizados
# Modelo binary cross-entropy, solo se reduce la devianza
modelo_tc %>% 
  layer_dense(units = 10, activation = 'sigmoid', 
              kernel_regularizer = regularizer_l2(l = 1e-3), #regularizer_l2 es ridge
              kernel_initializer = initializer_random_uniform(minval = -0.5, maxval = 0.5),
              input_shape=7) %>% #variables iniciales
  layer_dense(units = 10, activation = 'sigmoid', 
...
iteraciones <- modelo_tc %>% fit(
  x_ent_s, y_ent, 
  #batch size mismo que nrow(x_ent_s) es descenso en gradiente.
...
iteraciones <- modelo_tc %>% fit(
  ...
  epochs = 500, batch_size = nrow(x_ent_s), #numero de iteraciones epochs = 500
  verbose = 0, #verbose = 1, muestra los resultados en consola
