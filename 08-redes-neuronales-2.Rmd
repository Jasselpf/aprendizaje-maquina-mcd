...
se le llama un **minilote** **(minibatch)**. En cada iteración, nos movemos en 
...
```
**Note** Generalmente los minilotes - minibatch son del mismo tamaño siempre, para poder movernos con el mismo factor.
Los pasos se deben hacer mas chicos en comparacion con desenso en gradiente
...
```{r}
iter_estocastico <- descenso_estocastico(30, rep(0, 4), 0.1, minilotes) %>%
...
estimadas.
**Note** Generalmente con potencia de 2: 32, 64, 128. Aprovechar el procesamiento de los GPUs.
Una epoca es correr todos los minilotes
...
```{r}
iter_estocastico <- descenso_estocastico(20, c(0,0, 0, 0), 0.3, 
...
```{r}
set.seed(231)
iter_estocastico <- descenso_estocastico(20, c(0,0, 0, 0), 0.2, minilotes, momento = 0.7, decaimiento = 0.001) %>%
  data.frame %>% rename(beta_0 = X1, beta_1 = X2, beta_2=X3, beta_3=X4)
dev_ent <- devianza_calc(x = as.matrix(dat_ent[,c('x_1','x_2','x_3'), drop =FALSE]), 
                             y=dat_ent$g)
dev_valid <- devianza_calc(x = as.matrix(dat_valid[,c('x_1','x_2','x_3'), drop =FALSE]), 
...
**Note** Cuando hay muchas neuronas es conveniente utilizar etas o factores iniciales muy chicos
Inicializar con valores muy chicos.

```{r}
modelo <- keras_model_sequential() #Los valores iniciales estan definidos por default: uniforme de glorot
modelo %>%
  layer_dense(units = 1, 
              activation = 'sigmoid',
              input_shape = c(3))
...
modelo %>% compile(loss = 'binary_crossentropy',
                   optimizer = optimizer_sgd(lr = 0.2, momentum = 0, #lr - learning rate (estocastico)
                                             decay = 0),
  metrics = c('accuracy')
)
...
```
**Note** Devianza de entrenamiento es 0.39
...
coef(mod_logistico)
0.5*mod_logistico$deviance/nrow(dat_ent) #Devianza - glm da la suma de los errores
```
...
modelo_tc %>% 
  ...
  #Calcular probabilidades (la suma de todas las predicciones es 1) con la ultima capa - softmax (problemas de multivariada)
              kernel_regularizer = regularizer_l2(l = 1e-6))
```

**Note** Las activaciones de cada capa indicarían la probabilidad (prediccion). Se puede usar predict para obtener las probabilidades finales.

```{r}
modelo_tc %>% compile(
  loss = 'categorical_crossentropy',
  optimizer = optimizer_sgd(lr = 0.5, momentum = 0.0, decay = 1e-6), #decay es para ir rediciendo el tamaño de paso
  metrics = c('accuracy' ,'categorical_crossentropy')
)
history <- modelo_tc %>% fit(
  x_train, y_train, 
  epochs = 100, batch_size = 256, #100 epocas
  verbose = 0,
...
modelo_dropout  %>% 
  ...
  layer_dropout(0.5) %>% #Porcentaje de neuronas que se quitan
...
colnames(pesos) <- 1:ncol(pesos)
graf_pesos(pesos) 
get_weights(modelo_dropout)[[3]] #para ver los pesos de las capas
```
