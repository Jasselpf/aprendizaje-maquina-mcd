...
son num?ricos. Por ejemplo, si $G$ denota el estado del contrato de celular
de un cliente dentro de un a?o, podr?amos tener $G\in \{ activo, cancelado\}$.
...
(rojo, verde y azul). Si las im?genes son de 100x100, tendríamos 30,000 variables
de entrada.
...
### Error de clasificaci?n y funci?n de p?rdida 0-1
...
Vamos a construir el modelo de regresi?n log?stica (binaria) 
...
N?tese que intentar estimar las probabilidades de clase $p_1(x)$ de forma lineal con

$$p_1(x)=\beta_0+\beta_1 x_1$$
tiene el defecto de que el lado derecho puede producir valores fuera
de $[0,1]$. La idea es entonces aplicar una funci?n $h$ simple
que transforme la recta real al intervalo $[0,1]:$
$$p_1(x) = h(\beta_0+\beta_1 x_1),$$
donde $h$ es una funci?n que toma valores en $[0,1]$. ?C?al es la funci?n
...
```{block2, type='comentario'}
La funci?n log?stica est? dada por
$$h(x)=\frac{e^x}{1+e^x}$$
``` 
NOTE: El calsificador de Bayes minimisa el error de clasificaci?n
```{r}
...
Esta funcion comprime adecuadamente (para nuestros prop?sitos) 
el rango de todos los reales dentro del intervalo $[0,1]$. Si aplicamos
al predictor lineal que consideramos, obtenemos:


```{block2, type='comentario'}
El modelo de regresi?n log?stica simple est? dado por
...
Este es un modelo param?trico con 2 par?metros.

#### Ejercicio {-}

- Demostrar que, si $p_1(x)$ est? dado como en la ecuaci?n anterior, entonces
...
#### Ejemplo {-}
En nuestro ejemplo, ten?amos el siguiente ajuste con k-vecinos m?s cercanos:
...
algunos modelos log?sticos para las probabilidades
de clase:
...
Podemos usar tambi?n la funci?n glm de R para ajustar los coeficientes:
...
```{block2, type='comentario'}
El modelo de regresi?n log?stica est? dado por
$$p_1(x)=p_1(x;\beta)= h(\beta_0+\beta_1x_1 + \beta_2x_2 +\cdots + \beta_p x_p),$$
y $$p_0(x)=p_0(x;\beta)=1-p_1(x;\beta),$$
donde $\beta=(\beta_0,\beta_1, \ldots, \beta_p)$.
```

## Aprendizaje de coeficientes para regresi?n log?stica (binomial).
...
Igual que en regresi?n lineal, comenzamos por calcular la derivada de un t?rmino:
...
Podr?amos usar las siguientes implementaciones, que representan cambios
...
***Observaci?n** 
Sin embargo, podemos hacer una simplificaci?n para tener mejor desempe?o y estabilidad.
Observamos que 
...
Comparamos con glm:
NOTE: glm utiliza Newton Rapson para llegar al m?nimo, segundas derivadas.
...
N?tese que esta devianza est? calculada sin dividirentre el n?mero de casos. Podemos calcular la devianza promedio de entrenamiento haciendo:
...
As? que ajustar el modelo minimizando la expresi?n
\@ref(eq:devianza)
...
#### Desempe?o de regresi?n log?stica como m?todo de aprendizaje {-}
Igual que en regresi?n lineal, regresi?n log?stica supera a m?todos
...
**Ojo**: En regresi?n lineal, las variables contribuyen independientemente
de otras al predictor. Eso no pasa en regresi?n log?stica debido a la no linealidad
introducida por la funci?n log?stica $h$. Por ejemplo, imaginemos el modelo:
...
Ahora calculamos devianza de prueba y error de clasificaci?n:
