...
**Note class** Este caso es malo porque se sobre ajusta a los datos de entrenamiento

Podemos ver todos los modelos en una misma gráfica y apreciar mejor la
...
**Note class** Le quitamos el beta cero, porque el modelo original no tiene beta cero
...
**Note class* Optimizacion convexa - revisar
...
Si escogemos un valor relativamente grande de $\lambda$, entonces terminaremos con una solución donde los coeficientes $\beta_j$ no pueden alejarse mucho de 0, 
...
## Regularización ridge (o cordillera)
...
**Note class* Del lado izquierdo es la zona de sobre ajuste o alta varianza.
Del lado del izqierdo es la zona del sesgo.
Error de prediccion = sesgo + varianza + error irreducible
...
**Note class** La correlación en las variables afecta o se traduce en un problema de varianza en las betas o estimadores, y eso a su vez produce varianza en las predicciones de $\hat(y)$
...
**Note class** El modelo final es el promedio de los 5 (o k) modelos/separaciones.
El modelo extremo es LOO-CV (leave one out cross validation), si tienes 1000 datos, dejas solo un valor afuera, ajustas un modelo con 999, y validas con el que dejaste afuera.
...
cv_mod_ridge <- cv.glmnet(x = x_e, y=dat_e$y, 
  alpha = 0,
  family='binomial', intercept = F, nfolds = 10, nlambda=50) ##nfolds - separaciones
plot(cv_mod_ridge)
cv_mod_ridge$lambda.min
cv_mod_ridge$lambda.1se
```

**Note class** Es tradicional escoger lamdas entre las dos rectas inicadas arriba.
El punto rojo es el promedio de todas las muestras al fijar lambda. Las cajitas indican la varianza de todas las separaciones muestras al fijar lambda.
...
**Note class** Cross validation generalmente se utiliza como metodo para seleccionar hiper-parametros, como lamba.
...
**Note class** Elastic net = combinacion de ambos metodos --> Lasso y Ridge
